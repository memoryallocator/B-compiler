...
use crate::parser::{extract_bracketed_expression, Parse, ParseExact};
        ...
        Self: Sized,
    {
        ...
        fn parse_primary_expressions(
            ...
            while let Some(t) = input.get(i) {
                ...
                match &t.token {
                    WrappedToken::Name(name) => toks_or_prim_or_br_exprs.push(
                        TokenOrPrimaryExpr::PrimaryExpr(PrimaryExprAndPos {
                            prim_expr: Box::new(PrimaryExpr::Name(name.clone())),
                            position: pos,
                        }),
                    ),

                    WrappedToken::Constant(constant) => toks_or_prim_or_br_exprs.push(
                        TokenOrPrimaryExpr::PrimaryExpr(PrimaryExprAndPos {
                            ...
                            toks_or_prim_or_br_exprs.push(TokenOrPrimaryExpr::PrimaryExpr(
                                PrimaryExprAndPos {
                                    prim_expr: match br_expr {
                                        BracketedExpr::Round(single_arg) => {
        ...
        fn parse_unary_operators_and_reverse(
            ...
            fn unary_op_can_be_applied(
                ...
                if !(if prefix_or_postfix == PrefixOrPostfix::Postfix {
                    op_may_be_postfix
                } else {
                    op_may_be_prefix
                }(op))
                ...
                match op {
                    Operator::Plus | Operator::Minus | Operator::Asterisk | Operator::Unary(_) => {
                        true
                    }
            ...
            fn apply_unary_op(
                ...
                let rvalue = match op {
                    ...
                    Operator::Ampersand => {
                        Rvalue::TakeAddress(LvalueNode::try_from(&rvalue_node).unwrap())
                    }
                ...
                match vec.last() {
                    Some(TokenOrRvalueNode::RvalueNode(rvalue_node)) => {
                        if unary_op_can_be_applied(op, rvalue_node, prefix_or_postfix) {
                            ...
                            Err(OpCannotBeApplied {
                                op_pos,
                                expr_pos: Some(rvalue_node.position),
                            })
                        }
                    }
                    Some(TokenOrRvalueNode::Token(t)) => Err(OpCannotBeApplied {
                        op_pos,
                        expr_pos: Some(t.pos),
                    }),
                    None => Err(OpCannotBeApplied {
                        op_pos,
                        expr_pos: None,
                    }),
                }
            ...
            let bin_op_priority = HashMap::<Operator, i32>::from_iter(
                {
...

                    {
                        let multiplicative_ops =
                            map_priority(vec![Asterisk, Binary(Div), Binary(Mod)], 100);

                        res.extend(multiplicative_ops);
                    }

                    {
                        let additive_ops = map_priority(vec![Plus, Minus, Binary(Mod)], 90);
                    ...
                    {
                        let shift_ops =
                            map_priority(vec![Binary(Shift(Left)), Binary(Shift(Right))], 80);
                    ...
                    {
                        let eq_ops = map_priority(vec![Binary(Cmp(Eq)), Binary(Cmp(Ne))], 60);
                ...
                Ok(RvalueNode {
                    ...
                    rvalue: Box::new(Rvalue::Binary {
                        ...
                        bin_op: match op {
                            ...
                            Binary(bin_op) => RegularBinary(bin_op),
                            _ => unreachable!(),
                        },
                        rhs,
                    }),
                ...
                match t_or_rv {
                    TokenOrRvalueNode::Token(t) => {
                        ...
                        if let Some(op) = op {
                            ...
                            let ops_applied = free_ops_and_operands_stacks(
                                &mut bin_ops_and_positions,
                                &mut operands,
                            )?;
                    ...
                    TokenOrRvalueNode::RvalueNode(rv_node) => {
                        ...
                        let next_op = if let Some(TokenOrRvalueNode::Token(next_tok)) =
                            reversed_input.last()
                        {
                        ...
                        while let Some((curr_op, _)) = bin_ops_and_positions.last() {
                            if let Some(next_op) = next_op {
                                if bin_op_priority.get(&next_op) > bin_op_priority.get(curr_op) {
                                    ...
                                    let op_applied = apply_last_bin_op(
                                        &mut bin_ops_and_positions,
                                        &mut operands,
                                    )?;
                                    operands.push(op_applied);
        ...
        fn parse_conditional_expressions_and_assignments(
            ...
            fn parse_assignments(
                ...
                while input.len() > 1 {
                    ...
                    let (lhs_rv, lhs_pos) = match input.pop_back() {
                        ...
                        None => {
                            return Err(vec![NoOperandForOperator(
                ...
                match input.pop_back().unwrap() {
                    ...
                    AssignOrRvalueNode::Assign(_, pos) => Err(vec![UnexpectedToken(pos)]),
                }
            }
            ...
            match input.len() {
                ...
                1 => match input.pop() {
                    ...
                    Some(TokenOrRvalueNode::Token(t)) => Err(vec![UnexpectedToken(t.pos)]),
                    _ => unreachable!(),
                },
                _ => Err(vec![UnexpectedToken(match input.pop().unwrap() {
